...
def run_scraper(
    ...
    PrintUtils.print_bg_color(
        f"Run scraper (multithreading={use_multithreading})",
        bg_color
    )
    ...
    print("\n")
    PrintUtils.print_bg_color(
        f"Execution time (multithreading={use_multithreading}): {duration:.2f} seconds",
        bg_color
    )
    print("\n")
    return duration
...
def main():
    ...
    exec_time_thread_true = run_scraper(
        use_multithreading=use_multithreading,
        output_format=OutputFormat.JSON_AND_CSV
    )

    # Run with multithreading disabled
    use_multithreading = False
    exec_time_thread_false = run_scraper(
        use_multithreading=use_multithreading,
        output_format=OutputFormat.JSON_AND_CSV
    )
    ...
    PrintUtils.print_bg_color(
        f"- With multithreading:    {exec_time_thread_true:.2f} seconds",
        BgColor.BLUE
    )
    PrintUtils.print_bg_color(
        f"- Without multithreading: {exec_time_thread_false:.2f} seconds",
        BgColor.GREEN
    )
    ...
    if exec_time_thread_false > exec_time_thread_true:
        ...
        PrintUtils.print_color(
            "\nMultithreading was slower — the API might be blocking concurrent "
            "requests.\nSequential execution may be more efficient when servers "
